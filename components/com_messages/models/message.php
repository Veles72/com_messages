	...
	public function getItem($pk = null)
	{
		if (!isset($this->item))
		{
			...
			if ($this->item->user_id_from && $fromUser = new JUser($this->item->user_id_from)) {
                ...
                // Устанавливаем дефолтные тему и сообщение
                $application = JFactory::getApplication();
		$subject = $application->getUserState('com_messages.subject',NULL);
		$message = $application->getUserState('com_messages.message',NULL);
                if(isset($subject))
                {
                    $this->item->set('subject', $subject);
                    $subject = $application->setUserState('com_messages.subject',NULL);
                }
                if(isset($message))
                {
                    $this->item->set('message', $message);
                    $message = $application->setUserState('com_messages.message',NULL);
                }
